#!/usr/bin/env python3
"""
移动端展示层 CPT 发版工具（支持 JSX，UI 库为 antd-mobile）

与 PC 端 display_writer.py 的差异：
    1. 骨架模板指向 base_cpt_page_mobile.cpt
    2. Hook 解构转换识别 antdMobile.useState（PC 版只识别 antd.useState）
    3. DEVELOPER ZONE 注入位置在 bootBusiness() 函数体内
    4. 质量门 display_mobile/display_checker.py，含移动专属规则

工具链流程（与 PC 完全一致）：
    1. .jsx → esbuild 编译 → .mjs
    2. 去除注释（保留字符串）
    3. Hook 解构转换（React/antd/antdMobile.useState）
    4. node --check 语法门
    5. 注入 base_cpt_page_mobile.cpt 骨架
    6. 质量门检查
    7. 原子落盘

用法：
    python display_writer.py --jsx pages/my_page.jsx --output page.cpt
...
_HOOKS_DESTRUCT_RE = re.compile(
    r'const\s*\[\s*(\w+)\s*,\s*(\w+)\s*\]\s*=\s*(React|antd|antdMobile)\.useState\(([\s\S]*?)\);',
    re.MULTILINE
)
...
def transform_hooks_destructuring(js: str) -> str:
    """
    将 React.useState / antd.useState / antdMobile.useState 的解构赋值
    转换为两步赋值，解决帆软填报预览环境下批量更新失效的问题。

    转换前：
        const [value, setValue] = React.useState('');
        const [count, setCount] = antdMobile.useState(0);
    ...
    """
    _info("转换 React.useState / antdMobile.useState 解构赋值（帆软环境兼容）……")
...
def write(output_path: str, mjs_path: str, *, skip_check: bool = False):
    ...
    # 移动骨架采用「标记字符串替换」模式：
    # CDATA 内已有完整 PREAMBLE/POSTAMBLE，开发者代码注入到 bootBusiness() 内
    # 注入标记：/* @FRM_DEVELOPER_ZONE@ */
    with open(_BASE_TEMPLATE, "r", encoding="utf-8") as fh:
        template_str = fh.read()

    INJECT_MARKER = "/* @FRM_DEVELOPER_ZONE@ */"
    if INJECT_MARKER not in template_str:
        _die(f"骨架模板缺少注入标记 {INJECT_MARKER}，模板可能已损坏")

    new_str = template_str.replace(INJECT_MARKER, clean)
    _ok("已注入到骨架的 bootBusiness() 内（标记替换模式）")
