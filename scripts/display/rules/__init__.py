...
def _iter_rule_modules():
    ...
    _disp_pkg  = sys.modules.setdefault("display", type(sys)("display"))
    _disp_pkg.__path__ = [os.path.dirname(_RULES_DIR)]

    _rules_pkg = sys.modules.setdefault("display.rules", type(sys)("display.rules"))
    _rules_pkg.__path__ = [_RULES_DIR]
    _rules_pkg.__package__ = "display"
    ...
    for fname in sorted(os.listdir(_RULES_DIR)):
        ...
        short_name = fname[:-3]                                        # "cpt_xml_wellformed"
        full_name  = f"display.rules.{short_name}"                   # "display.rules.cpt_xml_wellformed"
